...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!agreed) {
      setError('Please agree to not keep all secrets!')
      return
    }

    setLoading(true)

    const { error } = await signUp(email, password, username, selectedAvatar)
        ...
        <div className="relative p-8 rounded-3xl shadow-2xl"
          ...
          <div className="w-full">
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              ...
              {/* Avatar Selection */}
              <div className="w-full">
                <p className="text-sm font-bold text-white text-center mb-2">Choose Your Avatar:</p>
                <div className="flex justify-center gap-2 flex-wrap">
                  {avatars.map((avatar, index) => (
                    <button
                      ...
                      className={`w-16 h-16 rounded-full overflow-hidden border-4 transition-all transform hover:scale-110 ${
                        selectedAvatar === avatar
                          ? 'border-pink-500 ring-4 ring-pink-300 scale-110'
              ...
              <div className="flex items-center justify-center gap-2 py-1">
                ...
                <label htmlFor="agree" className="text-xs font-medium text-gray-700">
                  I agree to not keep all secrets!
                </label>
              </div>
